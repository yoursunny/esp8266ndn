                              ...
                              uint8_t curve_type)
{
  ...
  pub_abs_key->key_size = (uECC_BYTES*2);
  prv_abs_key->key_size = uECC_BYTES;
  return NDN_SUCCESS;
}
                              ...
                              const struct abstract_ecc_pub_key* abs_key, uint8_t ecdsa_type)
{
  ...
  raw_signature_size = (uECC_BYTES*2);
  ...
  if (uECC_verify(abs_key->key_value, input_value,
                  raw_sig_temp_buf) == 0) {
...
#else
  ecc_sign_result = uECC_sign(abs_key->key_value, input_value,
                              output_value);
#endif
