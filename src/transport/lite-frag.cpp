...
#define LOG(...) LOGGER(LiteFrag, __VA_ARGS__)
...
LiteFrag::receive(uint8_t* buf, size_t bufSize, uint64_t& endpointId)
{
  ...
  while (true) {
    ...
    if (seq == 0) {
      if (m_offset != 0) {
        LOG(F("discard-incomplete id=") << m_id << F(" frags=") << m_nextSeq);
      ...
      LOG(F("drop-ooo want=(") << m_id << "," << m_nextSeq <<
                   F(") got=(") << id << "," << seq << ")");
      continue;
    }
...
LiteFrag::reassemble(uint8_t* buf, size_t bufSize)
{
  ...
  LOG(F("reassemble id=") << m_id << F(" frags=") << nFrags);
  return size;
}
...
LiteFrag::send(const uint8_t* pkt, size_t len, uint64_t endpointId)
{
  ...
  for (size_t offset = 0; offset < len;) {
    ...
    if (err != NDN_ERROR_success) {
      LOG(F("send id=") << id << F(" seq=") << seq << F(" err=") << err);
      return err;
    }
    yield();
  }
