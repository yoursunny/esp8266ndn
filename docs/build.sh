...
BASEURI=https://esp8266ndn.ndn.today
NS0=ndnph
NS1=esp8266ndn
...
find html -maxdepth 1 -name '*.html' | while read -r F; do
    ...
    ANALYTICS='<script async src="https://www.googletagmanager.com/gtag/js?id=G-YSW3MP43Z4"></script><script>window.dataLayer=[];function gtag(){dataLayer.push(arguments);}if(location.hostname.endsWith(".ndn.today")){gtag("js",new Date());gtag("config","G-YSW3MP43Z4");}</script>'
...
find html -maxdepth 1 \
    \( -name 'index.html' -o -name 'annotated.html' \
    -o -name "namespace$NS0*.html" -o -name "class$NS0*.html" -o -name "struct$NS0*.html" \
    -o -name "namespace$NS1*.html" -o -name "class$NS1*.html" -o -name "struct$NS1*.html" \
